...
% OUTPUT: FIGURE 1B (Probe trajectories following execution and imagery trials of an exemplary participant (S18)
%========================================================================================================================
...
%% READ IN DATA
allSubjNr = [18]; % exemplary participant
data_AllSubj      = [];
...
for i = allSubjNr
    if i <= 9
        load([datapath  '\' 'A061_Data_complete_S0' num2str(i)])
    else
        load([datapath  '\' 'A061_Data_complete_S' num2str(i)])
    end
...
for s = 1:size(data,2)
        ...
        if data(i,s).Error_Any == 0 && data(i,s).blocks_thisRepN ~= 0 && ( isnan(data(i,s).num_peak_vel_Prime) || data(i,s).num_peak_vel_Prime <=2 ) && data(i,s).num_peak_vel_Probe <=2 %no error trials and no practice trials and only smooth trajectories
            if strcmp(data(i,s).stop_signal_prime,'stop') && strcmp(data(i,s).obstacle_probe,'yes')
                if strcmp(data(i,s).obstacle_prime,'no')
                    color = hex2rgb('#9C9C9C');
                elseif strcmp(data(i,s).obstacle_prime,'yes')
                    color = hex2rgb('#000080'); %dark blue
                end
                patchline(data(i,s).mouse_probe_x_filt(data(i,s).mov_onset_Probe:data(i,s).frameNum_probe_time_cursor_in_target),data(i,s).mouse_probe_y_filt(data(i,s).mov_onset_Probe:data(i,s).frameNum_probe_time_cursor_in_target),'Linewidth',linewidth,'EdgeColor',color,'edgealpha',alpha), hold on
                obs_plot   = plot(data(i,s).mouse_probe_x_filt(data(i,s).mov_onset_Probe:data(i,s).mov_onset_Probe),data(i,s).mouse_probe_y_filt(data(i,s).mov_onset_Probe:data(i,s).mov_onset_Probe),'Linewidth',linewidth,'color', hex2rgb('#000080'));
                noobs_plot = plot(data(i,s).mouse_probe_x_filt(data(i,s).mov_onset_Probe:data(i,s).mov_onset_Probe),data(i,s).mouse_probe_y_filt(data(i,s).mov_onset_Probe:data(i,s).mov_onset_Probe),'Linewidth',linewidth,'color', hex2rgb('#9C9C9C'));
                xlim([-10 10])
                ylim([-10 10])
    ...
    h_label1=ylabel('Motor Imagery','visible','on','FontSize',25, 'FontWeight','bold');
    ...
    if data(1,s).subID <= 9
        exportgraphics(Fig_Probe_Traj,[figure_path, '\A061_ProbeTraj_S0', num2str(data(1,s).subID),'.pdf'],'ContentType','vector')
        saveas(Fig_Probe_Traj,[figure_path, '\A061_ProbeTraj_S0' num2str(data(1,s).subID) '.png'])
    else
        exportgraphics(Fig_Probe_Traj,[figure_path, '\A061_ProbeTraj_S', num2str(data(1,s).subID),'.pdf'],'ContentType','vector')
        saveas(Fig_Probe_Traj,[figure_path, '\A061_ProbeTraj_S' num2str(data(1,s).subID) '.png'])
    end
end
