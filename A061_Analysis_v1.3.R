# Title: Distinct contributions of motor imagery and execution to history-dependent biases in reaching (Experiment 1, A061)
...
pp_check(A061_fit.IRE_Probe_NoObs_OPRIxSS,   type = "dens_overlay", ndraws = 100)  # looks good
pp_check(A061_fit.IRE_Probe_NoObs_OPRIxSS,   type = "hist", ndraws = 10)           # looks good
pp_check(A061_fit.IRE_Probe_NoObs_OPRIxSS,   type = "boxplot", ndraws = 10)        # looks good


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.IRE_Probe_NoObs_OPRIxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.IRE_Probe_NoObs_OPRIxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
#  ==== POSTERIOR PREDICTIVE CHECKS ====
pp_check(A061_fit.IRE_Probe_Obs_OPRIxSS,   type = "dens_overlay", ndraws = 100)  # good
pp_check(A061_fit.IRE_Probe_Obs_OPRIxSS,   type = "hist", ndraws = 10)           # 
pp_check(A061_fit.IRE_Probe_Obs_OPRIxSS,   type = "boxplot", ndraws = 10)        # 


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.IRE_Probe_Obs_OPRIxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.IRE_Probe_Obs_OPRIxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
g.IRE.EMM.A061 <-
  (g.IRE_Obs.EMM.A061 + g.IRE_NoObs.EMM.A061) +
  plot_layout(
    widths = c(1, 1.2),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_InitialReachError_EMM.pdf")
...
#outfile <- file.path(figurePath, "A061_InitialReachError_EMM.png")
...
( RR_Obs   <- rope_range(A061_fit.IRE_Probe_Obs_OPRIxSS) )
...
titleX <- "Reach Bias (°)"
titleY <- ""
g.IRE_NoObs.Contrast.A061 <- IRE.Contrasts_long %>% filter(contrast=="Execution NoObs" | contrast=="Motor Imagery NoObs") %>% 
  ...
  annotate("rect", xmin = RR_NoObs[1], xmax = RR_NoObs[2], ymin = -0.1, ymax = 0.7, fill = color_Neut, alpha = .25)  +
...
titleX <- "Reach Bias (°)"
titleY <- ""
g.IRE_Obs.Contrast.A061 <- IRE.Contrasts_long %>% filter(contrast=="Execution Obs" | contrast=="Motor Imagery Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
  ...
  annotate("rect", xmin = RR_Obs[1], xmax = RR_Obs[2], ymin = -0.1, ymax = 0.7, fill = color_Neut, alpha = .25)  +
  geom_vline(xintercept = 0, linewidth = 1, linetype = 'dashed') +
...
titleY <- ""
g.IRE_NoObs_EXvsMI.Contrast.A061 <- IRE.Contrasts_long %>% filter(contrast=="EX minus MI NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
...
titleX <- expression(Delta~Reach~Bias)
titleY <- ""
g.IRE_Obs_EXvsMI.Contrast.A061 <- IRE.Contrasts_long %>% filter(contrast=="EX minus MI Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
...
#merge into a 1x2 plot grid (library patchwork)
g.IRE.Contrast.A061 <-
  (g.IRE_Obs.Contrast.A061 + g.IRE_NoObs.Contrast.A061 + g.IRE_Obs_EXvsMI.Contrast.A061 + g.IRE_NoObs_EXvsMI.Contrast.A061) +
  plot_layout(
    widths = c(1, 25/15), heights = c(2,1),
    guides = "collect"
  ) +
  plot_annotation(tag_levels = "A") &
  theme(legend.position = "bottom")
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_InitialReachError_Contrasts_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.IRE.Contrast.A061,
...
#outfile <- file.path(figurePath, "A061_InitialReachError_Contrasts_ALL.png")
...
titleX <- "Reach Bias (°)"
titleY <- ""
g.IRE.Contrast2.A061 <- IRE.Contrasts_long %>% filter(contrast=="Execution" | contrast=="Motor Imagery") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution" = "Execution",
...
titleX <- expression(Delta~Reach~Bias)
titleY <- ""
g.IRE_EXvsMI.Contrast2.A061 <- IRE.Contrasts_long %>% filter(contrast=="EX minus MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
...
#merge into a 1x2 plot grid (library patchwork)
g.IRE.Contrast_Pooled.A061 <-
  (g.IRE.Contrast2.A061 + g.IRE_EXvsMI.Contrast2.A061) +
  plot_layout(
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_InitialReachError_ContrastsPooled_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.IRE.Contrast_Pooled.A061,
...
#outfile <- file.path(figurePath, "A061_InitialReachError_ContrastsPooled_ALL.png")
...
#  ==== POSTERIOR PREDICTIVE CHECKS ====
pp_check(A061_fit.RT_Probe_OPRIxOPROxSS,   type = "dens_overlay", ndraws = 100)  # looks good
pp_check(A061_fit.RT_Probe_OPRIxOPROxSS,   type = "hist", ndraws = 10)           # looks good
pp_check(A061_fit.RT_Probe_OPRIxOPROxSS,   type = "boxplot", ndraws = 10)        # looks good


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.RT_Probe_OPRIxOPROxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.RT_Probe_OPRIxOPROxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
#merge into a 1x2 plot grid (library patchwork)
g.RT.EMM.A061 <-
  (g.RT_Obs.EMM.A061 + g.RT_NoObs.EMM.A061) +
  plot_layout(
    widths = c(1, 1.2),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_RT_Probe_EMM.pdf")
...
#outfile <- file.path(figurePath, "A061_RT_Probe_EMM.png")
...
#### Calculate ROPE
( RR       <- rope_range(A061_fit.RT_Probe_OPRIxOPROxSS) )
...
titleY <- ""
g.RT_NoObs.Contrast.A061 <- RT.Contrasts_long %>% filter(contrast=="Execution NoObs" | contrast=="Motor Imagery NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution NoObs" = "Execution",
    "Motor Imagery NoObs" = "Motor Imagery"
...
g.RT_NoObs.Contrast.A061


# Probe with obstacle
titleX <- "RT difference (°)"
titleY <- ""
g.RT_Obs.Contrast.A061 <- RT.Contrasts_long %>% filter(contrast=="Execution Obs" | contrast=="Motor Imagery Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution Obs" = "Execution",
    "Motor Imagery Obs" = "Motor Imagery"
  ...
  geom_point(data=dplyr::filter(RT.subj.contrast, probe == "with obstacle"),
             ...
             show.legend=FALSE) +
  scale_color_manual(labels = c(""),values = c(color_exe1,color_mi1)) +
  scale_x_continuous(name= titleX, limits=c(-100,10),breaks=c(-100,-50,0,50)) + 
  scale_y_continuous(limits = c(-0.02, 0.15)) +
  theme_cowplot() + custom_plot_theme +
  #theme(strip.text.y = element_blank()) + #remove facet labels
  labs(y= NULL) + guides(y = "none") +    #remove y-axis
...
titleY <- ""
g.RT_NoObs_EXvsMI.Contrast.A061 <- RT.Contrasts_long %>% filter(contrast=="EX minus MI NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI NoObs" = "EX minus MI"
  ))) + 
...
g.RT_NoObs_EXvsMI.Contrast.A061
...
titleX <- expression(Delta~RT~difference~(ms))
titleY <- ""
g.RT_Obs_EXvsMI.Contrast.A061 <- RT.Contrasts_long %>% filter(contrast=="EX minus MI Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI Obs" = "EX minus MI"
  ))) + 
...
g.RT_Obs_EXvsMI.Contrast.A061



#merge into a 1x2 plot grid (library patchwork)
g.RT.Contrast.A061 <-
  (g.RT_Obs.Contrast.A061 + g.RT_NoObs.Contrast.A061 + g.RT_Obs_EXvsMI.Contrast.A061 + g.RT_NoObs_EXvsMI.Contrast.A061) +
  plot_layout(
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_RT_Probe_Contrasts_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.RT.Contrast.A061,
#  width = 24, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_RT_Probe_Contrasts_ALL.png")
...
titleX <- "RT difference (°)"
titleY <- ""
g.RT.Contrast2.A061 <- RT.Contrasts_long %>% filter(contrast=="Execution" | contrast=="Motor Imagery") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution" = "Execution",
    "Motor Imagery" = "Motor Imagery"
  ...
  geom_point(data=dplyr::filter(RT.subj.contrast_pooled),
             ...
             show.legend=FALSE) +
  scale_color_manual(labels = c(""),values = c(color_exe1,color_mi1)) +
  scale_x_continuous(name= titleX, limits=c(-15,75),breaks=c(-50,-25,0,25,50,75)) + 
  scale_y_continuous(limits = c(-0.02, 0.15)) +
  theme_cowplot() + custom_plot_theme +
  #theme(strip.text.y = element_blank()) + #remove facet labels
  labs(y= NULL) + guides(y = "none") +    #remove y-axis
...
titleX <- expression(Delta~RT~difference~(ms))
titleY <- ""
g.RT_EXvsMI.Contrast2.A061 <- RT.Contrasts_long %>% filter(contrast=="EX minus MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI" = "EX minus MI"
  ))) + 
  ...
  geom_point(data=dplyr::filter(RT.subj.diffContrast_pooled),
             ...
             show.legend=FALSE) +
  scale_color_manual(labels = c(""),values = c(color_exe_mi1)) +
  scale_x_continuous(name= titleX, limits=c(-15,75),breaks=c(-25,0,25,50,75)) + 
  scale_y_continuous(limits = c(-0.02, 0.15)) +
  theme_cowplot() + custom_plot_theme +
  #theme(strip.text.y = element_blank()) + #remove facet labels
  labs(y= NULL) + guides(y = "none") +    #remove y-axis
...
g.RT_EXvsMI.Contrast2.A061



#merge into a 1x2 plot grid (library patchwork)
g.RT.Contrast_Pooled.A061 <-
  (g.RT.Contrast2.A061 + g.RT_EXvsMI.Contrast2.A061) +
  plot_layout(
    widths = c(1), heights = c(2,1),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_RT_Probe_ContrastsPooled_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.RT.Contrast_Pooled.A061,
#  width = 18, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_RT_Probe_ContrastsPooled_ALL.png")
...
#  ==== POSTERIOR PREDICTIVE CHECKS ====
pp_check(A061_fit.FRE_Probe_NoObs_OPRIxSS,   type = "dens_overlay", ndraws = 100)  # 
pp_check(A061_fit.FRE_Probe_NoObs_OPRIxSS,   type = "hist", ndraws = 10)           # 
pp_check(A061_fit.FRE_Probe_NoObs_OPRIxSS,   type = "boxplot", ndraws = 10)  # 


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.FRE_Probe_NoObs_OPRIxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.FRE_Probe_NoObs_OPRIxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
##### for comparison only: truncated gaussian model way better than lognormal: BF:6.59e+178
#( startTime <- Sys.time() )
#A061_fit.FRE_Probe_NoObs_OPRIxSS_LOGNORMAL <- brm( 
#  bf(EndPointError_abs_Probe | trunc(ub = 4) ~ obstacle_prime * stop_signal_prime + ( obstacle_prime * stop_signal_prime | subID )),     # model specification
#  data   = dClean_noobs,              # data
#  family = lognormal(),               # distribution of the response variable
#  prior  = prior_FRE,                 # prior distributions of model parameters
#  sample_prior = TRUE,                # we also consider the data now and save samples from prior
...
#  control = list(adapt_delta = 0.9, max_treedepth = 10),
#  seed =  1234,                       # set seed for reproducability
#  file = 'A061_fit.FRE_Probe_NoObs_OPRIxSS_LOGNORMAL', #save model
#)
#endTime <- Sys.time()
#( endTime- startTime )

#pp_check(A061_fit.FRE_Probe_NoObs_OPRIxSS_LOGNORMAL,   type = "dens_overlay", ndraws = 100)  # 

#comparison <- bayesfactor_models(A061_fit.FRE_Probe_NoObs_OPRIxSS_LOGNORMAL, A061_fit.FRE_Probe_NoObs_OPRIxSS, denominator = A061_fit.FRE_Probe_NoObs_OPRIxSS_LOGNORMAL)
#comparison
...
#  ==== POSTERIOR PREDICTIVE CHECKS ====
pp_check(A061_fit.FRE_Probe_Obs_OPRIxSS,   type = "dens_overlay", ndraws = 100)  # looks ok
pp_check(A061_fit.FRE_Probe_Obs_OPRIxSS,   type = "hist", ndraws = 10)           # looks ok
pp_check(A061_fit.FRE_Probe_Obs_OPRIxSS,   type = "boxplot", ndraws = 10)        # looks ok


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.FRE_Probe_Obs_OPRIxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.FRE_Probe_Obs_OPRIxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
#merge into a 1x2 plot grid (library patchwork)
g.FRE.EMM.A061 <-
  (g.FRE_Obs.EMM.A061 + g.FRE_NoObs.EMM.A061) +
  plot_layout(
    widths = c(1, 1.2),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_FinalReachError_EMM.pdf")
...
#outfile <- file.path(figurePath, "A061_FinalReachError_EMM.png")
...
#### Calculate ROPE
( RR_NoObs <- rope_range(A061_fit.FRE_Probe_NoObs_OPRIxSS) )
( RR_Obs   <- rope_range(A061_fit.FRE_Probe_Obs_OPRIxSS) )
( RR       <- ( RR_NoObs + RR_Obs)/2 )
...
#  ==== FINAL REACH ERROR: PLOTTING CONTRASTS  ====
# Probe without obstacle
titleX <- "Final Reach Bias (°)"
titleY <- ""
g.FRE_NoObs.Contrast.A061 <- FRE.Contrasts_long %>% filter(contrast=="Execution NoObs" | contrast=="Motor Imagery NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution NoObs" = "Execution",
    "Motor Imagery NoObs" = "Motor Imagery"
  ...
  annotate("rect", xmin = RR_NoObs[1], xmax = RR_NoObs[2], ymin = -2, ymax = 20, fill = color_Neut, alpha = .25)  +
  geom_vline(xintercept = 0, linewidth = 1, linetype = 'dashed') +
...
titleX <- "Final Reach Bias (°)"
titleY <- ""
g.FRE_Obs.Contrast.A061 <- FRE.Contrasts_long %>% filter(contrast=="Execution Obs" | contrast=="Motor Imagery Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution Obs" = "Execution",
    "Motor Imagery Obs" = "Motor Imagery"
  ...
  annotate("rect", xmin = RR_Obs[1], xmax = RR_Obs[2], ymin = -2, ymax = 20, fill = color_Neut, alpha = .25)  +
  geom_vline(xintercept = 0, linewidth = 1, linetype = 'dashed') +
...
titleY <- ""
g.FRE_NoObs_EXvsMI.Contrast.A061 <- FRE.Contrasts_long %>% filter(contrast=="EX minus MI NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI NoObs" = "EX minus MI"
  ))) + 
...
titleX <- expression(Delta~Final~Reach~Bias~(degree))
titleY <- ""
g.FRE_Obs_EXvsMI.Contrast.A061 <- FRE.Contrasts_long %>% filter(contrast=="EX minus MI Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI Obs" = "EX minus MI"
  ))) + 
...
#merge into a 1x2 plot grid (library patchwork)
g.FRE.Contrast.A061 <-
  (g.FRE_Obs.Contrast.A061 + g.FRE_NoObs.Contrast.A061 + g.FRE_Obs_EXvsMI.Contrast.A061 + g.FRE_NoObs_EXvsMI.Contrast.A061) +
  plot_layout(
    widths = c(1, 1/1), heights = c(2,1),
    guides = "collect"
  ) +
  plot_annotation(tag_levels = "A") &
  theme(legend.position = "bottom")
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_FinalReachError_Contrasts_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.FRE.Contrast.A061,
#  width = 24, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_FinalReachError_Contrasts_ALL.png")
...
titleX <- "Final Reach Bias (°)"
titleY <- ""
g.FRE.Contrast2.A061 <- FRE.Contrasts_long %>% filter(contrast=="Execution" | contrast=="Motor Imagery") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution" = "Execution",
    "Motor Imagery" = "Motor Imagery"
...
titleX <- expression(Delta~Final~Reach~Bias~(degree))
titleY <- ""
g.FRE_EXvsMI.Contrast2.A061 <- FRE.Contrasts_long %>% filter(contrast=="EX minus MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI" = "EX minus MI"
  ))) + 
...
g.FRE_EXvsMI.Contrast2.A061



#merge into a 1x2 plot grid (library patchwork)
g.FRE.Contrast_Pooled.A061 <-
  (g.FRE.Contrast2.A061 + g.FRE_EXvsMI.Contrast2.A061) +
  plot_layout(
    widths = c(1), heights = c(2,1),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_FinalReachError_ContrastsPooled_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.FRE.Contrast_Pooled.A061,
#  width = 18, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_FinalReachError_ContrastsPooled_ALL.png")
...
#  ==== POSTERIOR PREDICTIVE CHECKS ====
pp_check(A061_fit.EPE150_Probe_OPRIxOPROxSS,   type = "dens_overlay", ndraws = 100)  # looks good
pp_check(A061_fit.EPE150_Probe_OPRIxOPROxSS,   type = "hist", ndraws = 10)           # looks good
pp_check(A061_fit.EPE150_Probe_OPRIxOPROxSS,   type = "boxplot", ndraws = 10)        # looks good


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.EPE150_Probe_OPRIxOPROxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.EPE150_Probe_OPRIxOPROxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
#merge into a 1x2 plot grid (library patchwork)
g.EPE150.EMM.A061 <-
  (g.EPE150_Obs.EMM.A061 + g.EPE150_NoObs.EMM.A061) +
  plot_layout(
    widths = c(1, 1.2),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPE150_EMM.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPE150.EMM.A061,
#  width = 18, height = 12, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPE150_EMM.png")
...
#### Calculate ROPE
( RR       <- rope_range(A061_fit.EPE150_Probe_OPRIxOPROxSS)*10 ) # convert from cm to mm
...
#  ==== ACCURACY: PLOTTING CONTRASTS OVERALL EXECUTION VS MOTOR IMAGERY ====
titleX <- expression(Delta~Endpoint~Error~(mm))
titleY <- "EX minus MI"
g.EPE150_EXvsMI.Contrast.A061 <- EPE150.Contrasts_long %>% filter(contrast=="EX vs MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPE150_EXminusMI_Contrast.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPE150_EXvsMI.Contrast.A061,
#  width = 24, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPE150_EXminusMI_Contrast.png")
...
#  ==== ACCURACY:: PLOTTING CONTRASTS ====
# Probe without obstacle
titleX <- expression(Delta~Endpoint~Error~(mm))
titleY <- ""
g.EPE150_NoObs.Contrast.A061 <- EPE150.Contrasts_long %>% filter(contrast=="Execution NoObs" | contrast=="Motor Imagery NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution NoObs" = "Execution",
    "Motor Imagery NoObs" = "Motor Imagery"
...
titleX <- expression(Delta~Endpoint~Error~(mm))
titleY <- ""
g.EPE150_Obs.Contrast.A061 <- EPE150.Contrasts_long %>% filter(contrast=="Execution Obs" | contrast=="Motor Imagery Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution Obs" = "Execution",
    "Motor Imagery Obs" = "Motor Imagery"
...
titleY <- ""
g.EPE150_NoObs_EXvsMI.Contrast.A061 <- EPE150.Contrasts_long %>% filter(contrast=="EX minus MI NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI NoObs" = "EX minus MI"
  ))) + 
...
titleX <- expression(Delta~Endpoint~Error~Difference(mm))
titleY <- ""
g.EPE150_Obs_EXvsMI.Contrast.A061 <- EPE150.Contrasts_long %>% filter(contrast=="EX minus MI Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI Obs" = "EX minus MI"
  ))) + 
...
#merge into a 1x2 plot grid (library patchwork)
g.EPE150.Contrast.A061 <-
  (g.EPE150_Obs.Contrast.A061 + g.EPE150_NoObs.Contrast.A061 + g.EPE150_Obs_EXvsMI.Contrast.A061 + g.EPE150_NoObs_EXvsMI.Contrast.A061) +
  plot_layout(
    widths = c(1, 1/1), heights = c(2,1),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPE150_Contrasts_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPE150.Contrast.A061,
#  width = 24, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPE150_Contrasts_ALL.png")
...
titleX <- expression(Delta~Endpoint~Error~(mm))
titleY <- ""
g.EPE150.Contrast2.A061 <- EPE150.Contrasts_long %>% filter(contrast=="Execution" | contrast=="Motor Imagery") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution" = "Execution",
    "Motor Imagery" = "Motor Imagery"
...
titleX <- expression(Delta~Endpoint~Error~Difference(mm))
titleY <- ""
g.EPE150_EXvsMI.Contrast2.A061 <- EPE150.Contrasts_long %>% filter(contrast=="EX minus MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI" = "EX minus MI"
  ))) + 
...
g.EPE150_EXvsMI.Contrast2.A061



#merge into a 1x2 plot grid (library patchwork)
g.EPE150.Contrast_Pooled.A061 <-
  (g.EPE150.Contrast2.A061 + g.EPE150_EXvsMI.Contrast2.A061) +
  plot_layout(
    widths = c(1), heights = c(2,1),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPE150_ContrastsPooled_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPE150.Contrast_Pooled.A061,
#  width = 18, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPE150_ContrastsPooled_ALL.png")
...
#  ==== POSTERIOR PREDICTIVE CHECKS ====
pp_check(A061_fit.EPP150_Probe_OPRIxOPROxSS,   type = "dens_overlay", ndraws = 100)  # looks good
pp_check(A061_fit.EPP150_Probe_OPRIxOPROxSS,   type = "hist", ndraws = 10)           # looks good
pp_check(A061_fit.EPP150_Probe_OPRIxOPROxSS,   type = "boxplot", ndraws = 10)        # looks good


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.EPP150_Probe_OPRIxOPROxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.EPP150_Probe_OPRIxOPROxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
#merge into a 1x2 plot grid (library patchwork)
g.EPP150.EMM.A061 <-
  (g.EPP150_Obs.EMM.A061 + g.EPP150_NoObs.EMM.A061) +
  plot_layout(
    widths = c(1, 1.2),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPP150_EMM.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPP150.EMM.A061,
#  width = 18, height = 12, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPP150_EMM.png")
...
#### Calculate ROPE
( RR       <- rope_range(A061_fit.EPP150_Probe_OPRIxOPROxSS) ) 
...
#  ==== PRECISION: PLOTTING CONTRASTS OVERALL EXECUTION VS MOTOR IMAGERY ====
titleX <- expression(Delta~Ellipse~Area~(mm^2))
titleY <- "EX minus MI"
g.EPP150_EXvsMI.Contrast.A061 <- EPP150.Contrasts_long %>% filter(contrast=="EX vs MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX vs MI" = "EX minus MI"
  ))) + 
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPP150_EXminusMI_Contrast.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPP150_EXvsMI.Contrast.A061,
#  width = 24, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPP150_EXminusMI_Contrast.png")
...
#  ==== PRECISION: PLOTTING CONTRASTS ====
# Probe without obstacle
titleX <- expression(Delta~Ellipse~Area~(mm^2))
titleY <- ""
g.EPP150_NoObs.Contrast.A061 <- EPP150.Contrasts_long %>% filter(contrast=="Execution NoObs" | contrast=="Motor Imagery NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution NoObs" = "Execution",
    "Motor Imagery NoObs" = "Motor Imagery"
...
titleX <- expression(Delta~Ellipse~Area~(mm^2))
titleY <- ""
g.EPP150_Obs.Contrast.A061 <- EPP150.Contrasts_long %>% filter(contrast=="Execution Obs" | contrast=="Motor Imagery Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution Obs" = "Execution",
    "Motor Imagery Obs" = "Motor Imagery"
...
titleY <- ""
g.EPP150_NoObs_EXvsMI.Contrast.A061 <- EPP150.Contrasts_long %>% filter(contrast=="EX minus MI NoObs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI NoObs" = "EX minus MI"
  ))) + 
...
titleX <- expression(Delta~Ellipse~Area~Difference~(mm^2))
titleY <- ""
g.EPP150_Obs_EXvsMI.Contrast.A061 <- EPP150.Contrasts_long %>% filter(contrast=="EX minus MI Obs") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI Obs" = "EX minus MI"
  ))) + 
...
#merge into a 1x2 plot grid (library patchwork)
g.EPP150.Contrast.A061 <-
  (g.EPP150_Obs.Contrast.A061 + g.EPP150_NoObs.Contrast.A061 + g.EPP150_Obs_EXvsMI.Contrast.A061 + g.EPP150_NoObs_EXvsMI.Contrast.A061) +
  plot_layout(
    widths = c(1, 1/1), heights = c(2,1),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPP150_Contrasts_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPP150.Contrast.A061,
#  width = 24, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPP150_Contrasts_ALL.png")
...
titleX <- expression(Delta~Ellipse~Area~(mm^2))
titleY <- ""
g.EPP150.Contrast2.A061 <- EPP150.Contrasts_long %>% filter(contrast=="Execution" | contrast=="Motor Imagery") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution" = "Execution",
    "Motor Imagery" = "Motor Imagery"
...
titleX <- expression(Delta~Ellipse~Area~Difference~(mm^2))
titleY <- ""
g.EPP150_EXvsMI.Contrast2.A061 <- EPP150.Contrasts_long %>% filter(contrast=="EX minus MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI" = "EX minus MI"
  ))) + 
...
g.EPP150_EXvsMI.Contrast2.A061



#merge into a 1x2 plot grid (library patchwork)
g.EPP150.Contrast_Pooled.A061 <-
  (g.EPP150.Contrast2.A061 + g.EPP150_EXvsMI.Contrast2.A061) +
  plot_layout(
    widths = c(1), heights = c(2,1),
    guides = "collect"
  ) +
...
## Build a valid path
#outfile <- file.path(figurePath, "A061_EPP150_ContrastsPooled_ALL.pdf")

## Save as PDF (base pdf device)
#ggsave(
#  filename = outfile,
#  plot = g.EPP150.Contrast_Pooled.A061,
#  width = 18, height = 18, units = "cm",
#  device = grDevices::pdf,       # or simply rely on the .pdf extension
#  dpi = 300,                     # ignored for pdf, but harmless
#  limitsize = FALSE,
...
#outfile <- file.path(figurePath, "A061_EPP150_ContrastsPooled_ALL.png")
...
# ==== DESIGN AND EXPORT CORRELATION TABLE (SUPPLEMENTARY TABLE S1) ====
# combine dataframes
...
ftable.MI <- flextable(table.MI)  %>%  
  ...
  set_caption("Table S1: Correlations between Reach Bias and Motor Imagery Questionnaire Scores in Experiment 1") 
...
print(doc, target = "TableS1_A061_Table_MIQ_ReachBias_Correlations.docx")
...
# ==== MAKE PLOT (5x2 plot grid, library patchwork) (SUPPLEMENTARY FIGURE S1) ====
g.corr_MI <- (
...
# Build a valid path
outfile <- file.path(figurePath, "FigS1_A061_Correlation_MIQ_ReachBias.pdf")
...
# Build a valid path
outfile <- file.path(figurePath, "FigS1_A061_Correlation_MIQ_ReachBias.svg")
...
outfile <- file.path(figurePath, "FigS1_A061_Correlation_MIQ_ReachBias.png")
...
summary(dClean$TRT_Prime) #two NA's present, 3 trials with TRT < 200, total of 11 trials removed here
...
# prior predictive checks for reasonable priors
( startTime <- Sys.time() )
A061_fit.TRT_Prime_OPRIxSS <- brm( 
  ...
  control = list(adapt_delta = 0.9, max_treedepth = 10),
  seed =  1234,                       # set seed for reproducability
  file = paste0(analysisPath,'/', 'A061_fit.TRT_Prime_OPRIxSS'), #save model
)
endTime <- Sys.time()
( endTime- startTime )
...
#  ==== POSTERIOR PREDICTIVE CHECKS ====
pp_check(A061_fit.TRT_Prime_OPRIxSS,   type = "dens_overlay", ndraws = 100)  # looks good
pp_check(A061_fit.TRT_Prime_OPRIxSS,   type = "hist", ndraws = 10)           # looks good
pp_check(A061_fit.TRT_Prime_OPRIxSS,   type = "boxplot", ndraws = 10)        # looks good


#  ==== CONVERGENCE AND HMC DIAGNOSTICS ====
summary(A061_fit.TRT_Prime_OPRIxSS)

# Trace and density plots for MCMC Samples of all relevant parameters
# chains should mix and look like hairy caterpillars
plot(A061_fit.TRT_Prime_OPRIxSS,          
     combo = c("dens", "trace"),
     ask   = FALSE
)
...
#### Calculate ROPE
( RR       <- rope_range(A061_fit.TRT_Prime_OPRIxSS) )
...
#  ==== TOTAL RESPONSE TIME PRIME: PLOTTING CONTRASTS  ====
# Contrasts: Prime with obstacle minus Prime without obstacle
titleX <- expression(Delta~Prime~duration~(ms))
titleY <- ""
g.TRT.Contrast.A061 <- TRT.Contrasts_long %>% filter(contrast=="Execution" | contrast=="Motor Imagery") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "Execution" = "Execution",
    "Motor Imagery" = "Motor Imagery"
...
titleY <- "EX minus MI"
g.TRT_EXvsMI.Contrast.A061 <- TRT.Contrasts_long %>% filter(contrast=="EX minus MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX minus MI" = "EX minus MI"
  ))) + 
  ...
  scale_x_continuous(name= titleX, limits=c(-100,100),breaks=c(-100,-50,0,50,100)) + 
  scale_y_continuous(limits = c(-0.005, 0.04)) +
  theme_cowplot() + custom_plot_theme +
  #theme(strip.text.y = element_blank()) + #remove facet labels
  labs(y= NULL) + guides(y = "none") +    #remove y-axis
...
titleX <- expression(Delta~Prime~duration~(ms))
titleY <- "EX minus MI"
g.TRT_EXvsMI_overall.Contrast.A061 <- TRT.Contrasts_long %>% filter(contrast=="EX vs MI") %>% 
  ggplot(aes(x = value, fill = contrast, pattern_fill = contrast)) +
  facet_wrap(~contrast, strip.position = "left", ncol = 1,labeller = as_labeller(c(
    "EX vs MI" = "EX minus MI"
  ))) + 
...
g.TRT_EXvsMI_overall.Contrast.A061
...
# ==== CORRELATION OF PRIME DURATION OF EXECUTION WITH MOTOR IMAGERY TRIALS ====
...
# ==== PRIME TRT: MAKE ONE FIGURE (FIGURE 3) =====
design <- "ABCE
           ABDE"
...
# Build a valid path
outfile <- file.path(figurePath, "Fig3_A061_TRT_Prime_ALL.pdf")
...
# Build a valid path
outfile <- file.path(figurePath, "Fig3_A061_TRT_Prime_ALL.svg")
...
outfile <- file.path(figurePath, "Fig3_A061_TRT_Prime_ALL.png")
